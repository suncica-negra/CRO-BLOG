...
mongoose.connect("mongodb+srv://@cluster0-wwrbr.mongodb.net/blog", {useNewUrlParser: true, useUnifiedTopology: true, useFindAndModify: false});
...
if(req.session.passport === undefined){
  var korisnik = "";
   res.render("about1", {aboutContent: aboutContent, korisnik: korisnik});
} else {
  var korisnik = req.session.passport.user;
   res.render("about", {aboutContent: aboutContent, korisnik: korisnik});
}
...
if(req.session.passport === undefined){
  var korisnik = "";
   res.render("contact1", {contactContent: contactContent, korisnik: korisnik});
} else {
  var korisnik = req.session.passport.user;
   res.render("contact", {contactContent: contactContent, korisnik: korisnik});
}
...
if(req.session.passport === undefined){
  var korisnik = "";
   res.render("login", {korisnik: korisnik});
} else {
  var korisnik = req.session.passport.user;
   res.render("compose", {korisnik: korisnik});
}
...
Comment.populate(post.comments, {path:"comments"}, function(err, comments){
...
if(req.session.passport === undefined){
    ...
    Post.findOne({_id: requestedPostId}, function(err, post){
      ...
      res.render("comment", {korisnik: korisnik, requestedPostId: requestedPostId, title: title});
      });
    } 
});
...
app.get("/posts/:postId/edit", function(req, res){
  ...
  if(req.session.passport.user === ""){
    Post.findOne({_id: requestedPostId}, function(err, post){
      ...
      res.render("compose2", {korisnik: korisnik, requestedPostId: requestedPostId, title: title, content: content});
    });
...
app.put("/posts/:postId/edit", function(req, res){
  const requestedPostId = req.params.postId;
    if (req.session.passport.user === ""){
          var korisnik = req.session.passport.user;
  Post.findOneAndUpdate({_id:requestedPostId}, { $set: { content: req.body.postBody, title: req.body.postTitle }}, {new: true}, function(err, comment){
      if(!err){
...
app.get("/posts/:postId/delete", function(req, res){
    ...
    if (req.session.passport.user === ""){
      ...
      Post.findById(requestedPostId, function(err, foundPost){
        res.render("delete2", {korisnik: korisnik, requestedPostId: requestedPostId, post: foundPost});
      });
    } else {
      res.redirect("/");
    }
...
app.delete("/posts/:postId/delete", function(req, res){
      const requestedPostId = req.params.postId;
      var korisnik = req.session.passport.user;
  Post.findOneAndDelete({_id:requestedPostId}, function(err, res){
       if(!err){
...
app.get("/posts/:postId/comment/:commentId/:commentAuthor/edit", function(req, res){
  ...
  if(req.session.passport.user === ""){
    ...
    Comment.findById(requestedCommentId, function(err, foundComment){
      ...
      res.render("edit", {korisnik: korisnik, requestedPostId: requestedPostId, requestedCommentId: requestedCommentId, requestedCommentAuthor: requestedCommentAuthor, comment: foundComment});
    });
  } else if (req.session.passport === undefined || req.session.passport.user != requestedCommentAuthor){
    res.render("forbiden", {korisnik: korisnik, requestedPostId: requestedPostId, korisnikKomentara: requestedCommentAuthor});
...
app.put("/posts/:postId/comment/:commentId/:commentAuthor/edit", function(req, res){
  ...
  if (req.session.passport.user === ""){
          var korisnik = req.session.passport.user;
  Comment.findOneAndUpdate({_id:requestedCommentId}, { $set: { content: req.body.content }}, {new: true}, function(err, comment){
      if(!err){
...
app.get("/posts/:postId/comment/:commentId/:commentAuthor/delete", function(req, res){
  ...
  if(req.session.passport.user === "") {
    ...
    Comment.findById(requestedCommentId, function(err, foundComment){
      ...
      res.render("delete", {korisnik: korisnik, requestedPostId: requestedPostId, requestedCommentId: requestedCommentId, requestedCommentAuthor: requestedCommentAuthor, comment: foundComment});
    });
  } else if (req.session.passport === undefined || req.session.passport.user != requestedCommentAuthor){
        res.render("forbiden", {korisnik: korisnik, requestedPostId: requestedPostId, korisnikKomentara: requestedCommentAuthor});
  } else {
...
app.delete("/posts/:postId/comment/:commentId/:commentAuthor/delete", function(req, res){
  ...
  if (req.session.passport.user === ""){
      var korisnik = req.session.passport.user;
      Comment.findOneAndDelete({_id:requestedCommentId}, function(err, res){
        if(!err){
...
app.get("/troskovi", function(req, res){
  ...
  if(req.session.passport === undefined){
  var korisnik = "";
   res.render("troskovi1", {korisnik: korisnik});
} else {
  var korisnik = req.session.passport.user;
   res.render("troskovi2", {korisnik: korisnik});
}
...
app.get("/foto", function(req, res){
  ...
  if(req.session.passport === undefined){
  var korisnik = "";
   res.render("foto1", {korisnik: korisnik});
} else {
  var korisnik = req.session.passport.user;
   res.render("foto2", {korisnik: korisnik});
}
  
});

app.get("/register", function(req, res){
  res.render("register", {korisnik: korisnik});
});
